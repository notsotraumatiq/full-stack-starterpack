...
function App() {
  ...
  const fetchAppointments = useCallback(async (physicianId: number) => {
    try {
      const response = await axiosInstance.get(`/physicians/${physicianId}`);
  ...
  return (
    <div className="flex justify-center items-center w-screen h-screen">
      <div className="h-1/2 w-1/2 flex gap-4 shadow-sm">
        <div className="bg-slate-200 h-full  rounded-s-lg px-8">
              ...
              {physicians.length > 0
                ? physicians.map((physician) => (
                    <li
                      ...
                      onClick={() => fetchAppointments(physician.id)}
                    >
